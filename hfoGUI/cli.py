...
def _process_dl_file(data_path: Path, set_path: Optional[Path], args: argparse.Namespace):
    ...
    params = {
        ...
        'gap_threshold': float(getattr(args, 'gap_threshold', 0.05)),
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    dl.add_argument('--gap-threshold', type=float, default=0.05, help='Gap in seconds for merging nearby detections (default: 0.05 = 50ms for ripples; use 0.5 for longer epileptic events)')
    ...
    # --- HFO Metrics Parser ---
    metrics_parser = sub.add_parser('metrics-batch', help='Compute HFO metrics from scores files')
    ...
    # --- Filter Scores Parser ---
    filter_parser = sub.add_parser('filter-scores', help='Filter scores by duration or criteria')
...
def _load_scores_from_txt(scores_path: Path):
    """Load HFO scores from a TSV file (as generated by detection CLI).
    
    Returns:
        tuple: (starts_ms, stops_ms, metadata_dict)
    """
    if not scores_path.exists():
        raise FileNotFoundError(f"Scores file not found: {scores_path}")
    
    try:
        df = pd.read_csv(scores_path, sep='\t', dtype=str)
    except Exception as e:
        raise ValueError(f"Failed to parse scores file {scores_path}: {e}")
    ...
    for col in df.columns:
        ...
        if 'start' in col_lower:
            start_col = col
        elif 'stop' in col_lower or 'end' in col_lower:
            stop_col = col
    
    if not start_col or not stop_col:
        raise ValueError(f"Could not find Start/Stop columns in {scores_path}")
    
    try:
        starts = df[start_col].astype(float).values
        stops = df[stop_col].astype(float).values
    except Exception as e:
        raise ValueError(f"Could not convert Start/Stop to numeric: {e}")
...
def _compute_hfo_metrics(starts_ms, stops_ms, recording_duration_sec, verbose=False):
    ...
    durations_ms = stops_ms - starts_ms
    count = len(starts_ms)
    recording_minutes = recording_duration_sec / 60.0 if recording_duration_sec else 0
    
    metrics = {
        'total_events': count,
        'recording_duration_minutes': recording_minutes,
        'event_rate_per_min': (count / recording_minutes) if recording_minutes > 0 else 0,
        'mean_duration_ms': float(np.mean(durations_ms)) if len(durations_ms) > 0 else 0,
        'median_duration_ms': float(np.median(durations_ms)) if len(durations_ms) > 0 else 0,
        'min_duration_ms': float(np.min(durations_ms)) if len(durations_ms) > 0 else 0,
        'max_duration_ms': float(np.max(durations_ms)) if len(durations_ms) > 0 else 0,
        'std_duration_ms': float(np.std(durations_ms)) if len(durations_ms) > 0 else 0,
    }
    
    if verbose:
        print(f"  Events: {count}")
        print(f"  Rate: {metrics['event_rate_per_min']:.2f} events/min")
        print(f"  Duration: {metrics['mean_duration_ms']:.2f} ± {metrics['std_duration_ms']:.2f} ms")
    
    return metrics


def _infer_data_duration(data_path: Path, verbose=False):
    """Infer recording duration from data file (EEG/EGF) in seconds."""
    try:
        raw_data, Fs = ReadEEG(str(data_path))
        duration_sec = len(raw_data) / float(Fs) if Fs else 0
        if verbose:
            print(f"  Loaded {data_path.name}: {len(raw_data)} samples @ {Fs} Hz = {duration_sec:.1f} sec")
        return duration_sec
    except Exception as e:
        if verbose:
...
def run_metrics_batch(args: argparse.Namespace):
    ...
    if scores_path.is_file():
        ...
        scores_files = sorted(scores_path.rglob("*_HIL.txt")) + sorted(scores_path.rglob("*_CON.txt")) + sorted(scores_path.rglob("*_DL.txt"))
    else:
        print(f"Error: Scores path does not exist: {scores_path}")
        return
    ...
    for scores_file in scores_files:
        try:
            session_name = scores_file.stem.rsplit('_', 1)[0]  # Remove method tag
            if args.verbose:
                print(f"\nProcessing: {scores_file.name}")
            ...
            if args.data:
                ...
                candidates = list(data_dir.glob(f"{session_name}.egf")) + list(data_dir.glob(f"{session_name}.eeg"))
                if candidates:
                    duration_sec = _infer_data_duration(candidates[0], verbose=args.verbose)
            
            if duration_sec == 0 and args.duration_min:
...
def run_filter_scores(args: argparse.Namespace):
    ...
    try:
        ...
        print(f"Error filtering scores: {e}")
        if args.verbose:
            import traceback
            traceback.print_exc()


__all__ = ['build_parser', 'run_hilbert_batch', 'run_ste_batch', 'run_mni_batch', 'run_consensus_batch', 'run_dl_batch', 'run_prepare_dl', 'run_train_dl', 'run_export_dl', 'run_metrics_batch', 'run_filter_scores']
...
def main(args=None):
    ...
    if args.command == 'hilbert-batch':
        ...
        run_export_dl(args)
    elif args.command == 'metrics-batch':
        run_metrics_batch(args)
    elif args.command == 'filter-scores':
        run_filter_scores(args)
    else:
        parser = build_parser()
        parser.print_help()
